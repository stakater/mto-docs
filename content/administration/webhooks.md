...
!!! note
    `tenant-ingress-hostname` and `tenant-route-hostname` only register when the host validation regex is valid. An invalid regex means the webhook is silently not registered and hostnames are not enforced, so verify the tenant's status after configuring host validation. See [Restricting Hostname per Tenant](../multi-tenancy/guides/host-validation.md).
