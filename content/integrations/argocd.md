...
To ensure each tenant has their own ArgoCD AppProjects, administrators must first specify the ArgoCD namespace in the [IntegrationConfig](../concepts/integration-config.md):
...
To prevent tenants from syncing ResourceQuota and LimitRange resources to their namespaces, administrators can specify these resources in the blacklist section of the ArgoCD configuration in the [IntegrationConfig](../concepts/integration-config.md):
