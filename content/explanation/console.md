...
![hibernationTable](../images/hibernationTable.png)
...
#### 1. Graphical Represnatation

##### a. Tenant Requests vs. Cluster Capacity

This section displays bar charts that compare the current resource requests from tenants (like CPU and memory) with the total available cluster capacity. The charts visually represent how much of the cluster’s resources are currently being utilized versus what is available, helping identify overutilization or underutilization scenarios.
...
![capacityPlanning](../images/capacityPlanning.png)

#### 3. Workerpool Details
...
![workerPool](../images/workerPool.png)

The workerpool labels dropdown is used to filter the table data and graph based on the selected labels. If the selected label is common amongst the worker nodes, the filtered result is a list of all the worker nodes that contain the selected labels and update graph for filtered worker nodes.

If the label is unqiue for any worker node, selecting the label will filter the specific worker nodes containing the label and update the graph based on the filtered results.

To clear the filter you can unselect applied labels and choose Apply Filter action. If no label is selected, it will ask you to proceed or cancel. If you choose to proceed the filters will be cleared returing the table and graph to orginal state.

![workerPoolFiltered](../images/workerPoolFiltered.png)
...
![requestDetails](../images/requestDetails.png)
