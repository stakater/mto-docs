...
| `templateOperatorV2` _[TemplateOperatorV2Config](#templateoperatorv2config)_ | Defines configuration for the template-operator-v2 component |  | Optional: \{\} <br /> |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `mode` _[DependencyMode](#dependencymode)_ |  | Managed | Enum: [Managed External Disabled] <br /> |
...
- [TemplateOperatorV2Config](#templateoperatorv2config)
...
| `Disabled` | DependencyModeDisabled indicates the dependency is not deployed at all.<br />Currently only supported by templateOperatorV2; the webhook rejects it<br />for the other components.<br /> |
...
#### TemplateOperatorV2Config



TemplateOperatorV2Config defines the configuration for the template-operator-v2 component
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `mode` _[DependencyMode](#dependencymode)_ | Mode selects how template-operator-v2 is provided. Disabled (default)<br />deploys nothing; Managed deploys and manages it. External is not<br />supported yet. | Disabled | Enum: [Managed External Disabled] <br />Optional: \{\} <br /> |
| `values` _[RawExtension](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.30/#rawextension-runtime-pkg)_ | Values allows customization of the template-operator-v2 Helm chart values when mode is Managed |  | Optional: \{\} <br /> |
...
| `deployedSandboxes` _object (keys:string, values:string)_ | DeployedSandboxes maps users to the sandbox namespaces created from spec. It is a<br />prune ledger: entries no longer implied by spec are deleted. See ManagedNamespaces<br />for the full list of namespaces belonging to the tenant |  |  |
| `deployedNamespaces` _string array_ | DeployedNamespaces lists the namespaces created from spec.namespaces. It is a prune<br />ledger: entries no longer in spec are deleted. See ManagedNamespaces for the full list |  |  |
| `managedNamespaces` _string array_ | ManagedNamespaces lists every namespace carrying the tenant label, whether created<br />from spec, self served by an owner or a sandbox. Sorted. Report only, never drives deletion |  |  |
...
| `ready` _boolean_ | Ready mirrors the Ready condition: true only when the last reconcile hit no<br />error and no sub sync left a warning condition |  | Optional: \{\} <br /> |
