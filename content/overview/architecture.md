...
|------|------|-------------|
| Tenant Controller | Deployment | Responsible for managing the creation, deletion, and updating of tenants in the cluster via [Tenant CRD](../multi-tenancy/concepts/tenant.md). |
| Namespace Controller | Deployment | Responsible for managing the creation, deletion, and updating of namespaces in the cluster. |
| Extensions Controller | Deployment | Enhances MTO's functionality by allowing integration with external services and manages extensions via [Extension CRD](../extensions/concepts/extensions.md). |
| Quota Integration Config Controller | Deployment | Manages 2 different CRDs in one controller, [Quota CRD](../multi-tenancy/concepts/quota.md), and [IntegrationConfig CRD](../multi-tenancy/concepts/integration-config.md). |
