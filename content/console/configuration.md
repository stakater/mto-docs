...
Administrators are the users and groups listed under `spec.accessControl.privileged` in the `IntegrationConfig`. MTO's default admin user, `mto@stakater.com`, is not on that list out of the box, so add it to `privileged.users` to give it administrator access in the console. See [Console Admin Access](../getting-started/installation/openshift.md#console-admin-access) for the patch. It is a first-login convenience; on a production cluster replace it with your own administrators and remove it, see [Production Readiness](../getting-started/production-readiness.md#remove-the-default-admin-login).
